...
class RestController extends CController	
{
	...
	public function actionUpdate($id)
	{
		...
		if($model instanceof Video) {
			...
			if($model->save())
				...
				$msg = "<h1>Error</h1>";
				$msg .= sprintf("Couldn't update Video");
				$msg .= "<ul>";
	...
	public function actionCreate()
	{
		...
		$ctrl = new ManageVideosController('video');
		$ctrl->actionUploadVideo();
...
/*		if($video->save()) {
			...
			$msg = "<h1>Error</h1>";
			$msg .= sprintf("Couldn't create Video");
			$msg .= "<ul>";
			foreach($video->errors as $attribute => $attr_errors) {
				$msg .= "<li>Attribute: $attribute</li>";
				$msg .= "<ul>";
				foreach($attr_errors as $attr_error)
					$msg .= "<li>$attr_error</li>";
	...
	public function actionView($id) {
				$result = Video::model()->findByPk($id);
		if(is_null($result))
	...
	private function _checkAuth() {
		...
		if($username && $password) {
			$user = YumUser::model()->find('LOWER(username)=?',array(
						strtolower($username)));
			if($user !==null && $user->superuser && $password == $user->password)
				return true;
		}
