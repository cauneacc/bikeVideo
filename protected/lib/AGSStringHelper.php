...
class AGSStringHelper{
	...
	function prepare_string($string, $url=FALSE){
		$ascii=($this->utf8_is_ascii($string)) ? TRUE : FALSE;
		$regex=($ascii === TRUE) ? '/[^A-Za-z0-9 ]+/' : '/[^\pL\pN\pZs]+/u';

		$string=preg_replace($regex, ' ', $string);
		$string=preg_replace('/\s\s+/', ' ', $string);
		$string=($ascii === TRUE) ? trim($string) : $this->utf8_trim($string);

		if($url === TRUE){
			if($ascii === TRUE){
				$string=strtolower($string);
			}else{
				$string=$this->utf8_strtolower($string);
				$string=$this->utf8_replace_accents($string);
			}
	...
	function prepare_tags($string){
		$ascii=($this->utf8_is_ascii($string)) ? TRUE : FALSE;
		$regex=($ascii === TRUE) ? '/[^A-Za-z0-9\, ]+/' : '/[^\pL\pN\pZ\,s]+/u';

		$string=preg_replace($regex, ' ', trim($string));
		$string=preg_replace('/\s\s+/', ' ', $string);

		return ($ascii === TRUE) ? strtolower(trim($string)) : $this->utf8_strtolower($this->utf8_trim($string));
	}
	...
	function utf8_strlen($str){
		...
		return ($this->utf8_is_ascii($str)) ? strlen($str) : strlen(utf8_decode($str));
	}

	function utf8_clean($str){
		if(is_array($str)){
			foreach($str as $key=>$value){
				$str[$this->utf8_strip_ascii_ctrl($key)]=$this->utf8_strip_ascii_ctrl($value);
			}
		}elseif(is_string($str) && $str != ''){
			$str=$this->utf8_strip_ascii_ctrl($str);
		}
	...
	function utf8_replace_accents($str, $case=0){
		if(!$this->utf8_is_ascii($str)){
			return $this->_transliterate_accents($str, $case);
		}
	...
	function utf8_strpos($str, $needle, $offset = NULL){
		if(MBSTRING){
			return (is_null($offset)) ? mb_strpos($str, $needle) : mb_strpos($str, $needle, $offset);
		}

		if($this->utf8_is_ascii($str)){
			return (is_null($offset)) ? strpos($str, $needle) : strpos($str, $needle, $offset);
		}

		if(is_null($offset)){
			return _utf8_strpos($str, $needle);
		}else{
			return _utf8_strpos($str, $needle, $offset);
		}
	}

	function utf8_substr($str, $offset, $length = NULL){
		if(MBSTRING){
			return (is_null($length)) ? mb_substr($str, $offset) : mb_substr($str, $offset, $length);
		}
		...
		if(is_null($str)){
			return _utf8_substr($str, $offset);
		}else{
			return _utf8_substr($str, $offset, $length);
		}
	}
	...
	function utf8_trim($str, $charlist = FALSE){
		...
		return _utf8_ltrim(_utf8_rtrim($str, $charlist), $charlist);
	}
	...
	function utf8_ltrim($str, $charlist = FALSE){
		...
		if($this->utf8_is_ascii($charlist)){
			return ltrim($str);
		}

		VF::load('multibyte.multibyte.ltrim');
		return _utf8_ltrim($str, $charlist);
	}
	...
	function utf8_rtrim($str, $charlist = FALSE){
		...
		return _utf8_rtrim($str, $charlist);
	}
	...
	function utf8_strtolower($string){
		...
		if($this->utf8_is_ascii($string)){
			return strtolower($string);
		}

		return _utf8_strtolower($string);
	}
	...
	function utf8_strtoupper($string){
		...
		return _utf8_strtoupper($string);
	}
...
//--------------------------------------------------------------------
	...
	function _transliterate_accents($str, $case = 0){
	...
	function _utf8_ltrim($str, $charlist=FALSE){
		if($charlist === FALSE){
			return ltrim($str);
		}
	...
	function _utf8_rtrim($str, $charlist=FALSE){
		if($charlist === FALSE){
			return ltrim($str);
		}
	...
	function _utf8_strpos($str, $needle, $offset = NULL){
		if(is_null($offset)){
			...
			if(count($ar) > 1){
				return $this->utf8_strlen($ar['0']);
			}

			return false;
		}else{
			if(!is_int($offset)){
				trigger_error('$this->utf8_strpos: Offset must be an integer', E_USER_ERROR);
				return false;
			}

			$str=$this->utf8_substr($str, $offset);
			if(false !== ($pos=$this->utf8_strpos($str, $needle))){
				return $pos + $offset;
			}
	...
	function _utf8_strtolower($string){
		...
		return $this->utf8_from_unicode($uni);
	}

	function _utf8_strtoupper($string){
	...
	function _utf8_substr($str, $offset, $length = NULL){
		...
		if($offset < 0){
			// see notes
			$strlen=strlen($this->utf8_decode($str));
